        ...
        <div class="col-lg-12 d-flex align-items-stretch">
            <div class="card w-100">
                <div class="card-body p-4">
                    ...
                    <form action="{{ route('reg.store',['id'=>'start']) }}" method="post" enctype="multipart/form-data">
                        @csrf
                        ...
                        <div class="form-group row mb-3">
                            <div class="col-md-6">
                                <label>Peserta</label>
                                <select class="choices form-select" name="peserta">
                                    <option value="">Pilih Peserta</option>
                                    @foreach ($da as $item)
                                        <option value="{{ $item->id }}" @selected(isset($document) && $document->heads->apply->id == $item->id)>
                                            {{ strtoupper($item->fullname) }}</option>
                                    @endforeach
                                </select>
                                @error('peserta')
                                    <div class='small text-danger text-left'>{{ $message }}</div>
